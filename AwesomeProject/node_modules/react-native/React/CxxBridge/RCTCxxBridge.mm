...
//    _jsThread = [[NSThread alloc] initWithBlock:^{
...
- (void)invalidate
{
  ...
  [self ensureOnJavaScriptThread:^{
    ...
    for (RCTModuleData *moduleData in strongSelf->_moduleDataByID) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:RCTBridgeDidInvalidateModulesNotification
      ...
      strongSelf->_reactInstance.reset();
      strongSelf->_jsMessageThread.reset();
    ...
    [strongSelf->_jsThread cancel];
    strongSelf->_jsThread = nil;
...
- (void)executeApplicationScript:(NSData *)script url:(NSURL *)url async:(BOOL)async
{
  ...
  [self _tryAndHandleError:^{
    ...
    auto reactInstance = strongSelf->_reactInstance;
    if (scriptType == ScriptTag::RAMBundle) {
      [strongSelf->_performanceLogger markStartForTag:RCTPLRAMBundleLoad];
      auto ramBundle = std::make_unique<JSIndexedRAMBundle>(sourceUrlStr.UTF8String);
      std::unique_ptr<const JSBigString> scriptStr = ramBundle->getStartupCode();
      [strongSelf->_performanceLogger markStopForTag:RCTPLRAMBundleLoad];
      [strongSelf->_performanceLogger setValue:scriptStr->size() forTag:RCTPLRAMStartupCodeSize];
      if (reactInstance) {
        auto registry =
...
#endif
    RCTProfileInit(strongSelf);

    [strongSelf enqueueJSCall:@"Systrace" method:@"setEnabled" args:@[ @YES ] completion:NULL];
  }];
}
...
- (void)stopProfiling:(void (^)(NSData *))callback
{
  ...
  [self ensureOnJavaScriptThread:^{
    ...
    [SSelf enqueueJSCall:@"Systrace" method:@"setEnabled" args:@[ @NO ] completion:NULL];
    RCTProfileEnd(SSelf, ^(NSString *log) {
